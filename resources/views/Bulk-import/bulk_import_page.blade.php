                ...
                <div class="col-12 float-sm-right">
                    <div class="card">
                            ...
                            <div class="row">
                                <div class="w-100">
                                    <div class="box-uploader">
                                        <form action="{{ route('bulk.import.all') }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <div class="main-import-container">
                                                <div class="year-select-box pb-3">
                                                    <select id="fin_year" name="fin_year" class="form-control">
                                                        @foreach($fyear as $year)
                                                            <option value="{{ $year->finyear }}" @if(Session::get('finyear') == $year->finyear) selected @endif>
                                                                {{ $year->finyear }}
                                                            </option>
                                                        @endforeach
                                                    </select>                                                    
                                                </div>
                                                <div class="quarter-select-box pb-3">
                        ...
                        @if (session()->has('import_errors'))
                            <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000000)" x-show="show">
