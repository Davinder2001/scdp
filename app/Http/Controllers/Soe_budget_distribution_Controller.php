...
class Soe_budget_distribution_Controller extends Controller
{
    ...
    public function get_distribution_majorhead_by_department(Request $request)
    {
        ...
        if ($request->department_id) {
            ...
            if (!$majorheadList->isEmpty()) {
                foreach ($majorheadList as $index => $majorhead) {
                    $selected = ($index === 0) ? ' selected' : '';
                    $majorheadHtml .= '<option value="'.$majorhead->id.'"'.$selected.'>'.$majorhead->complete_head.'</option>';
                }
    
                // Automatically select the first majorhead and load schemes and soes
                $firstMajorheadId = $majorheadList->first()->id;
    
                // Load schemes for the first majorhead
                $schemeList = Scheme_master::where('majorhead_id', $firstMajorheadId)->get();
                if (!$schemeList->isEmpty()) {
                    $schemeHtml = '';
                    foreach ($schemeList as $index => $scheme) {
                        $selected = ($index === 0) ? ' selected' : '';
                        $schemeHtml .= '<option value="'.$scheme->id.'"'.$selected.'>'.$scheme->scheme_name.'</option>';
                    }
    
                    // Load soes for the first majorhead and scheme
                    $firstSchemeId = $schemeList->first()->id;
                    $soeBudgetDistribution = Soe_budget_distribution::where("scheme_id", $firstSchemeId)
                                                                    ->where('majorhead_id', $firstMajorheadId)
                                                                    ->where('department_id', $request->department_id)
                                                                    ->get();
                    $ids = $soeBudgetDistribution->pluck('soe_id')->toArray();
    
                    $soeList = Soe_master::where('scheme_id', $firstSchemeId)
                                         ->whereNotIn('id', $ids)
                                         ->get();
    
                    if (!$soeList->isEmpty()) {
                        $soeHtml = '';
                        foreach ($soeList as $index => $soe) {
    ...
    public function get_distribution_scheme_by_majorhead(Request $request)
    {
        ...
        if ($request->majorhead_id) {
            ...
            if (!$schemeList->isEmpty()) {
                ...
                // Load soes for the selected majorhead and scheme
                $soeBudgetDistribution = Soe_budget_distribution::where("scheme_id", $request->scheme_id)
                                                                ->where('majorhead_id', $request->majorhead_id)
                                                                ->where('department_id', $request->department_id)
                                                                ->get();
                $ids = $soeBudgetDistribution->pluck('soe_id')->toArray();
    
                $soeList = Soe_master::where('scheme_id', $request->scheme_id)
                                     ->whereNotIn('id', $ids)
                                     ->get();
    ...
    public function get_distribution_soe_by_scheme(Request $request)
    {
        ...
        if ($request->scheme_id && $request->majorhead_id && $request->department_id) {
            $soeBudgetDistribution = Soe_budget_distribution::where("scheme_id", $request->scheme_id)
                                                            ->where('majorhead_id', $request->majorhead_id)
                                                            ->where('department_id', $request->department_id)
                                                            ->get();
            ...
            if (!$soeList->isEmpty()) {
                if ($soeList->count() === 1) {
                    $soeHtml = '<option value="'.$soeList->first()->id.'" selected>'.$soeList->first()->soe_name.'</option>';
                } else {
                    $soeHtml = '';
