...
class bulk_import_all implements ToModel, WithHeadingRow, WithValidation, SkipsEmptyRows, WithEvents
{
    use Importable;
    ...
    // Prepare the update array
        ...
        $get_dep_id = $department_id->id ?? '';
        $get_plan_id = $plan_id->id ?? '';
        $get_fin_yr_id = $finyear_id->id ?? '';
    ...
    if($quarter_data_all){ 
        ...
        $q_4_data_db = $quarter_data_all->q_4_data ?? null;
        ...
        if ($department_id && $majorhead_id && $schememaster_id && $soemaster_id && $finyear_id && $plan_id && $service_id && $sector_id && $subsector_id) {
            $this->lastRow = $lastRow; 

            $budget_allocation = new Soe_budget_allocation([
            ...
            $budget_distribution = new soe_budget_distribution([
                ...
                'plan_id'       => $plan_id->id,
                'data'          => null,
                'q_1_data'      => $update_q_Data['q_1_data'],
                'q_2_data'      => $update_q_Data['q_2_data'],
                'q_3_data'      => $update_q_Data['q_3_data'],
                'q_4_data'      => $update_q_Data['q_4_data'],
            ]);
            $budget_distribution->save();
        }
